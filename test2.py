...
def load_templates(folder: str):
    ...
    templates = []
    # โหลด CascadeClassifier ภายในฟังก์ชันเพื่อหลีกเลี่ยงปัญหาเธรด (แม้ว่าจะประกาศ global ไว้แล้ว)
    ...
    for p in sorted(paths):
        ...
        # 1. แปลงเป็น Grayscale (ภาพที่มาถึงจุดนี้อาจเป็นภาพที่ถูก resize และเป็น Grayscale แล้ว แต่โค้ดนี้ยังทำงานได้)
        if len(img.shape) == 3:
...
# ตัวแปรบันทึกเวลาบันทึกหน้า unknown เพื่อควบคุมความถี่
last_save_time = [0] 

# -------------------------- ส่วนตรวจจับใบหน้า --------------------------
def detect_face_templates(frame, threshold=0.5, scales=[0.8,0.9,1.0,1.1,1.2], min_size=50, save_unknown=True, save_interval=5):
    ...
    for i, (x,y,w,h) in enumerate(faces):
        ...
        # ปรับขนาดขยายเล็กน้อยเพื่อเพิ่มขอบให้กับ ROI 
        margin = 10
        y1, y2 = max(0, y - margin), min(gray.shape[0], y + h + margin)
        x1, x2 = max(0, x - margin), min(gray.shape[1], x + w + margin)
        ...
        # บันทึก Unknown Face และควบคุมความถี่ (ทุก 5 วินาที)
        if best_label == "Unknown" and save_unknown:
            if last_save_time[0] == 0 or (current_time - last_save_time[0] >= save_interval):
                ...
                print(f"Saved unknown face for alert: {filename}")
                last_save_time[0] = current_time # อัปเดตเวลาที่บันทึก
...
def upload_image():
    ...
    # --- ส่วนประมวลผลภาพหลังอัปโหลด (Grayscale + Resize) ---
    try:
...
def monitor_unknown_folder():
    ...
    และเรียกฟังก์ชันแจ้งเตือนด้วยอีเมลเมื่อมีไฟล์ใหม่ถูกสร้าง
    **✅ แก้ไข: ลบไฟล์เสมอหลังการแจ้งเตือน**
    """
    print("--- เริ่มต้นการมอนิเตอร์โฟลเดอร์ Unknown ---")
    # ใช้กลไกการจำลองการเฝ้าดูโดยการเปรียบเทียบรายการไฟล์
    ...
    while True:
        try:
            ...
            for filename in new_files:
                # ตรวจสอบว่าเป็นไฟล์ภาพที่ต้องการแจ้งเตือนหรือไม่
                if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                    ...
                    # 1. เรียกฟังก์ชันแจ้งเตือนด้วยอีเมล (ไม่ว่าผลลัพธ์จะเป็นอย่างไร)
                    send_notification_email(filename, file_path)
                    
                    # 2. ✅ ลบไฟล์เสมอหลังการแจ้งเตือน (ไม่ว่าอีเมลจะสำเร็จหรือไม่)
                    try:
                        os.remove(file_path)
                        # ใช้ 'ทำการประมวลผล' แทน 'ส่งแจ้งเตือนสำเร็จ' เพื่อรวมกรณีที่อีเมลล้มเหลว
                        print(f"🗑️ ลบไฟล์ Unknown Face **{filename}** หลังจากทำการประมวลผลเพื่อเคลียร์พื้นที่")
                    except Exception as e:
                        print(f"❌ Error ในการลบไฟล์ Unknown Face {filename}: {e}")
            
            # 3. อัปเดตรายการไฟล์ที่เคยเห็น: 
            # ให้อัปเดตใหม่เสมอ เพราะไฟล์ถูกลบไปแล้ว
            last_files = set(os.listdir(app.config['UNKNOWN_FOLDER']))
